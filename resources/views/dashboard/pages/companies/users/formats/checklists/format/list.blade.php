...
@section('title_page') 
    ...
    <a href="{{route('formats.checklists.create')}}" class="btn btn-primary" title="Nueva formato de chequeo"><i class="fa fa-plus"></i> </a>
@stop

@section('breadcrumbs') {!! Breadcrumbs::render('formats.checklists') !!} @stop
        ...
        <div class="col-sm-4 col-lg-3" id="{{ $format->id }}" data-toggle="tooltip" data-original-title="{{ $format->description }}" data-placement="bottom">
            <div class="media-items animation-fadeInQuick2" data-category="pdf">
                <div class="media-items-options text-right">
                    <a href="{{ route('formats.checklists.show', $format->id) }}" class="btn btn-xs btn-info">Ver</a>
                    <a href="{{ route('formats.checklists.edit', $format->id) }}" class="btn btn-xs btn-warning"><i class="fa fa-pencil"></i></a>
